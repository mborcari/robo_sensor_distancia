...
void setup() {
  ...
  servoSensor.attach(servoMotorPin); 
...
void loop() {
  ...
  delay (100);
  // Aguarda 100 milesugodos
  
  Distancia = le_distancia_sensor_ultrasonico();                          // Medindo a Distancia em CM.
  if (Distancia < 25) {                             // Se há obstáculo encontrado a menos de 20cm.
   direcao ();                                      // Se Frente estiver bloqueado, mude de direção
  }
  else if (Distancia >= 25)  {                      // Se o obstáculo for encontrado entre a mais de 40cm 
